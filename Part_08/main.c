...
static char *do_compile(char *filename) {
  ...
  if (O_dumptokens) {
    ...
    fclose(Infh);
    Infh = popen(cmd, "r");
    Line = 1; Linestart = 1; Putback = 0;
    Peektoken.token = 0; scan(&Thistoken);
  }

  if (O_verbose)
    fprintf(stderr, "compiling %s\n", filename);

  cg_file_preamble();		// Generate the output file preamble
...
static void usage(char *prog) {
  fprintf(stderr, "Usage: %s [-vcS] [-o outfile] ", prog);
  ...
  fprintf(stderr, "          one or more of tok,sym,ast, comma separated\n");
  exit(1);
}
...
int main(int argc, char *argv[]) {
  ...
  // Get any flag values
  while ((opt = getopt(argc, argv, "cSD:L:o:")) != -1) {
    switch (opt) {
    case 'c':
      O_assemble = true;
      O_keepasm = false;
