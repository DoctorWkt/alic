...
// Local variables
string Outfilename;		// Name of our output file
...
// not be modified
string alter_suffix(string str, const char suffix) {
  char * posn;
  string newstr;
...
string do_compile(const string filename) {
  int *errnoptr;
  char cmd[TEXTLEN];
...
string do_qbe(const string filename) {
  char cmd[TEXTLEN];
  int err;

  string outfilename = alter_suffix(filename, 's');
  if (outfilename == NULL) {
...
string do_assemble(const string filename) {
  char cmd[TEXTLEN];
  int err;

  string outfilename = alter_suffix(filename, 'o');
  if (outfilename == NULL) {
...
void do_link(const string outfilename, string *objlist) {
  uint cnt;
  int size = TEXTLEN;
  char cmd[TEXTLEN];
  char * cptr;
  ...
  cptr = cmd;
  cnt = cast(snprintf(cptr, size, "%s %s ", LDCMD, outfilename), uint);
  cptr = cptr + cnt;
  size = size - cnt;

  // Now append each object file
  while (*objlist != NULL) {
    cnt = cast(snprintf(cptr, size, "%s ", *objlist), uint);
    cptr = cptr + cnt;
    size = size - cnt;
    objlist++;
  }
...
public int main(int argc, const char **argv) {
  string outfilename = AOUT;
  string qbefile;
  string asmfile;
  string objfile;
  string objlist[MAXOBJ];
  int i; 
  int objcnt = 0;
  int opt;
  ...
  while (true) {
    ...
    case 'L':
      if (strstr(optarg, "tok") != NULL)
        O_dumptokens = true;
      if (strstr(optarg, "sym") != NULL)
        O_dumpsyms = true;
      if (strstr(optarg, "ast") != NULL)
        O_dumpast = true;
      if (strstr(optarg, "misc") != NULL)
        O_logmisc = true;
    case 'S':
      O_keepasm = true;
      O_assemble = false;
