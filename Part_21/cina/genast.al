...
public void check_bel(Sym * sym, ASTnode * list, int offset,
				bool is_element, int basetemp) {
  ASTnode *wide;
  Sym *memb;
  int i;
  int exprtemp;
  ...
  if (is_element==false && is_array(sym)) {
    ...
    for (i = 0; i < get_numelements(sym,0); i++) {
      check_bel(sym, list, offset + i * ty.size, true, basetemp);
      list = list.mid;
    }
  ...
  if (is_struct(ty)) {
    foreach memb(ty.memb, memb.next) {
      check_bel(memb, list, offset + memb.offset, false, basetemp);
      list = list.mid;
    }
