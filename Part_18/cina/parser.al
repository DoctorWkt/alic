...
void funcptr_declaration(char *typename);
  ...
  if (Thistoken.token == T_ASSIGN) {
	...
	fatal("Unknown base type in type declaration: %s\n",
      ...
      if (Thistoken.token == T_RANGE) {
        ...
        if (!is_integer(ptrtype))
          fatal("Can only apply a range to an integer type\n");

        // Ensure the range is compatible with the type
        if (lower < typemin[ptrtype.kind])
          fatal("%ld is outside the range of %s\n",
                lower, get_typename(ptrtype));
        if (upper > typemax[ptrtype.kind])
          fatal("%ld is outside the range of %s\n",
                upper, get_typename(ptrtype));
      }
