...
// not be modified
char *alter_suffix(const char *str, const char suffix) {
  char *posn;
  char *newstr;
...
char *do_compile(const char *filename) {
  int *errnoptr;
  char cmd[TEXTLEN];
...
char *do_qbe(const char *filename) {
  char cmd[TEXTLEN];
  int err;
...
void do_link(const char *outfilename, char **objlist) {
  uint cnt;
  int size = TEXTLEN;
  char cmd[TEXTLEN];
  char *cptr;
