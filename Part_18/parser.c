...
static void type_declaration(void) {
  ...
  if (Thistoken.token == T_ASSIGN) {
	...
	ty->kind= basetype->kind;
      ...
      if (basetype != ptrtype)
	new_type(TY_USER, ptrtype->size, basetype->is_unsigned,
		ptrtype->ptr_depth, typename, ptrtype);
    }
  } else {
    // No '=' sign
...
static ASTnode *statement_block(Sym * func) {
  ...
  if ((type(true) != NULL) || (Thistoken.token == T_CONST))
    d = declaration_stmts();
...
static ASTnode *postfix_variable(ASTnode * n) {
  ...
  case T_DOT:
    ...
    if (is_array(memb) || (is_struct(memb->type))) {
      n->type = memb->type;
    } else {
