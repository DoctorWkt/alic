...
enum {
  ...
  T_PUBLIC, T_EXTERN,					// 59

  // Structural tokens
  T_NUMLIT, T_STRLIT, T_SEMI, T_IDENT,			// 61
  T_LBRACE, T_RBRACE, T_LPAREN, T_RPAREN,		// 65
  T_COMMA, T_ELLIPSIS, T_DOT,				// 69
  T_LBRACKET, T_RBRACKET, T_COLON,			// 72
};
...
struct Sym {
  ...
  bool has_addr;	// Does the symbol have an address?
			// For functions: has the function already
  ...
  ASTnode *initlist;	// List of any initial values
...
enum {
  ...
  A_LOGAND, A_LOGOR, A_BEL, A_BOUNDS				// 45
};
