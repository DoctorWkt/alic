...
Sym *add_symbol(char *name, int symtype, Type *type, int visibility) {
  ...
  if (visibility != SV_LOCAL) {
    ...
    if (this != NULL) this->visibility= visibility;
  }
  return (this);
}
...
#if 0
// Recursively output an expression list of global values.
// Output the value at val.
static void gen_globvals(ASTnode *val) {
  ...
  // Do BELs recursively
  while (val != NULL && val->op == A_BEL) {
    gen_globvals(val->left);
    val= val->mid;
  }

  // After BEL processing, stop
  // if no next value
  if (val == NULL) return;

  // Output this value from the list.
  cgglobsymval(val->type, val);

  // And then do the next value
  gen_globvals(val->mid);
}
...
void gen_globsyms(void) {
  ...
  for (this = Globhead->head; this != NULL; this = this->next)
    if (this->name != NULL && this->symtype == ST_VARIABLE
        && this->visibility != SV_EXTERN) {
      // Start the symbol
      cgglobsym(this);

      // No initial value, output zeroes
      if (this->initlist == NULL)
	cgglobsymval(this->type, NULL);
      else {
	if (O_logmisc) {
          fprintf(Debugfh, "%s globsym:\n", this->name);
	  dumpAST(this->initlist, 0);
	}

        // Output the list of values
        gen_globvals(this->initlist);
      }
