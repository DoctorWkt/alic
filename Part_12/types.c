...
Type *pointer_to(Type *ty) {
  ...
  // We didn't find one, so make one and return it. XXX Fix 8 again!
  return(new_type(ty->kind, 8, ty->ptr_depth+1, ty->name, ty->basetype));
}
...
bool is_integer(Type * ty) {
  TypeKind k = ty->kind;
  if (ty->ptr_depth !=0) return(false);
