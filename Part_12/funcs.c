...
int add_function(ASTnode * func, ASTnode * paramlist, int visibility) {
  ...
  funcptr = add_symbol(func->strlit, ST_FUNCTION, func->type, visibility);
  ...
  if (funcptr == NULL) {
    ...
    // All OK. Return ifit was previously
    // declared with a statement block.
    // XXX Maybe change this to return the bool?
    if (funcptr->has_addr==true)
      return(1);
    else
      return(0);
  }
  ...
  // or mark it as a variadic function
  if (func->rvalue == true)
    funcptr->count= -1;
  else
...
void declare_function(ASTnode *f, int visibility) {
  ...
  if (add_function(f, f->left, visibility))
...
void gen_func_statement_block(ASTnode *s) {
  ...
  genAST(s);
  gen_func_postamble(Thisfunction->type);
}
