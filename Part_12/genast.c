...
int genAST(ASTnode * n) {
  ...
  case A_WHILE:
    gen_WHILE(n, 0); return(NOREG);
  case A_SWITCH:
    gen_SWITCH(n); return(NOREG);
  case A_LOGOR:
  case A_LOGAND:
  ...
  case A_FOR:
    ...
    gen_WHILE(n, label); return(NOREG);
  }

  // Load the left and right sub-trees into temporaries.
  // If we are a GLUE node with a true rvalue, this means
  // that the right sub-tree is the short assignment in
  // a FOR loop. Insert the continue label from the
  // Breaklabel stack between the sub-trees
  if (n->left)  lefttemp  = genAST(n->left);

  if ((n->op == A_GLUE) && (n->rvalue==true)) {
    if (Breakhead == NULL)
  ...
  case A_CONTINUE:
    ...
    return(NOREG);
  case A_SCALE:
    // XXX At some point, add an optimisation
  ...
  case A_BOUNDS:
    label= add_strlit(n->strlit);
    temp= add_strlit(Thisfunction->name);
...
static int gen_funccall(ASTnode *n) {
  ...
  if (numargs > 0) {
	...
	param->count=0;
      ...
      for (i=0, param= func->paramlist; param != NULL; i++, param= param->next) {
	...
	for (this=n->right; this != NULL; this= this->right) {
	  ...
	  if (!strcmp(func->name, "printf") && (this->left->type->kind == TY_FLT32))
	    this->left = widen_type(this->left, ty_flt64, 0);
	  if (func->count != -1)
	    this->left= widen_expression(this->left, param->type);
	  typelist[i]= this->left->type;
	  arglist[i]= genAST(this->left);
	  ...
	  if (!strcmp(func->name, "printf") && (this->type->kind == TY_FLT32))
	    this = widen_type(this, ty_flt64, 0);
	  if (func->count != -1)
