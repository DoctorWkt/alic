...
Sym *find_symbol(char *name) {
  ...
  for (this = Symhead; this != NULL; this = this->next) {
    ...
    // If this is a function, walk the parameter list
    if (this->symtype == ST_FUNCTION) {
...
void end_scope(void) {
  Symhead= Globhead; Globhead= NULL;
}
...
void dumpsyms(void) {
  ...
  for (this = Symhead; this != NULL; this = this->next) {
    if (this->name == NULL) {
      ...
      switch(this->symtype) {
       case ST_VARIABLE: fprintf(debugfh, "variable "); break;
       case ST_FUNCTION: fprintf(debugfh, "function "); break;
      }
