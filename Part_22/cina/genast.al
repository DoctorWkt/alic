...
void gen_IF(const ASTnode * n);
void gen_WHILE(const ASTnode * n, const int for_label);
void gen_SWITCH(const ASTnode * n);
void gen_local(const ASTnode * n);
int gen_funccall(const ASTnode * n);
void gen_try(const ASTnode * n);
int gen_ternary(const ASTnode * n);
int gen_logandor(const ASTnode * n);
int gen_cast(const ASTnode * n);
int gen_aarray(const ASTnode * n, const int exprtemp, const Type *ty);
int gen_exists(const ASTnode * n);
int gen_undef(const ASTnode * n);
int gen_aaiterstart(const ASTnode * n);
int gen_aanext(const ASTnode * n);
...
public int genAST(const ASTnode * n) {
  ...
  case A_BOUNDS:
    ...
    return (cgboundscheck(lefttemp, righttemp, label, temp));
  case A_VASTART:
    cg_vastart(n);
    return (NOTEMP);
  case A_VAEND:
...
void gen_local(const ASTnode * n) {
  ...
  cgaddlocal(n.ty, n.sym, size, makezero, is_array(n.sym));

  // Is this an aggregate variable?
  if (is_array(n.sym) || is_struct(n.sym.ty)) {
    ...
    if (n.left != NULL) {
      ...
      if (has_range(n.ty)) {
        functemp = add_strlit(Thisfunction.name, true);
        cgrangecheck(lefttemp, n.ty, functemp);
      }
...
ASTnode *fixup_argument(const Type* paramtype, const bool is_inout, ASTnode * node) {
...
void gen_SWITCH(const ASTnode * n) {
  ...
  temp = genAST(n.left);
  if (n.left.ty == pointer_to(ty_int8) || n.left.ty == ty_string) {
...
// a local variable
public ASTnode *check_bel(const Sym * sym, ASTnode * list,
			const int offset, const bool is_element,
			const int basetemp) {
  ASTnode *wide;
  Sym *memb;
  int i;
  ...
  if (is_element==false && is_array(sym)) {
    ty= value_at(sym.ty);
    foreach i (0 ... get_numelements(sym,0) - 1) {
  ...
  if (wide == NULL)
    fatal("Initialisation value for %s not of type %s, but type %s\n",
		sym.name, get_typename(ty), get_typename(list.ty));
