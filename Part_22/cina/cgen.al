...
// Generate a label
void cglabel(const int l) {
  fprintf(Outfh, "@L%d\n", l);
}

// Generate a string literal
void cgstrlit(const int label, string val, const bool is_const) {
  char ch;
...
// Ditto for stores
string qbe_storetype(const Type * ty) {
  int kind = ty.kind;
...
string qbe_loadtype(const Type * ty) {
  int kind = ty.kind;
...
string qbe_exttype(const Type * ty) {
  int kind = ty.kind;
...
void cg_func_preamble(const Sym * func) {
  Sym *this;
  string qtype;
...
void cg_func_postamble(const Type * ty) {
  fprintf(Outfh, "@END\n");
...
void cgglobsym(const Sym * sym, const bool make_zero) {
  int align;
  int size;
  int power = 1;
  Type *ty = sym.ty;
...
void cgglobsymval(const ASTnode * value, const int offset) {
  string qtype;
  int label;
...
// End a global symbol
void cgglobsymend(const Sym * sym) {
  fprintf(Outfh, " }\n");
}

// Load a boolean value (only 0 or 1)
// into the given temporary
void cgloadboolean(const int t, const int val, const Type * ty) {
...
int cgloadlit(inout Litval value, const Type * ty) {
  string qtype;
...
int cgbinop(const int t1, const int t2, string op, const Type * ty) {
  // Get the matching QBE type
  string qtype = qbetype(ty);
...
int cgcompare(const int op, const int t1, const int t2, const Type * ty) {
  // Get the matching QBE type
  string qtype = qbetype(ty);
  string cmpstr;
  int offset;
...
void cgjump_if_false(const int t1, const int label) {
...
int cgnot(const int t, const Type * ty) {
  // Get the matching QBE type
  string qtype = qbetype(ty);
...
int cginvert(const int t, const Type * ty) {
...
void cgaddlocal(const Type * ty, const Sym * sym, const int size, const bool makezero, const bool isarray) {
  int align = 8;
  int temp = cgalloctemp();
  int t2 = cgalloctemp();
...
int cgcall(const Sym * sym, const int numargs, const int excepttemp, const int *arglist,
	   const Type ** typelist) {
  int rettemp = NOTEMP;
  int functemp;
  int i;
...
void cgreturn(const int temp, const Type * ty) {
...
int cgloadglobstr(const int label) {
  // Get a new temporary
  int t = cgalloctemp();
...
int cgderef(const int t, const Type * ty) {
...
int cgstorderef(const int t1, const int t2, const Type * ty) {
  // Get the matching QBE type
...
int cgboundscheck(const int t1, const int counttemp, const int aryname, const int funcname) {
...
void cgmove(const int t1, const int t2, const Type * ty) {
  fprintf(Outfh, "  %%.t%d =%s copy %%.t%d\n", t2, qbetype(ty), t1);
}

// Allocate space for the variable argument list
void cg_vastart(const ASTnode *n) {
#ifdef CPU_riscv64
  int temp;
#endif
...
void cg_vaend(const ASTnode *n) {
  return;
}

int cg_vaarg(const ASTnode *n) {
  int t = cgalloctemp();
...
int cg_getaaval(const int arytemp, const int keytemp, const Type *ty) {
  int t1 = cgalloctemp();
  int t2 = cgalloctemp();
  string qtype = qbetype(ty);
...
void cg_setaaval(const int arytemp, const int keytemp, const int valtemp, const Type *ty) {
  int t= valtemp;
  string qtype;
...
int cg_existsaaval(const int arytemp, const int keytemp) {
  int t = cgalloctemp();
...
int cg_delaaval(const int arytemp, const int keytemp) {
  int t = cgalloctemp();
...
int cg_strhash(const int keytemp) {
  int t = cgalloctemp();
...
int cg_free_aarray(const Sym * sym) {
  int arytemp;
...
int cg_aaiterstart(const int arytemp) {
  int t = cgalloctemp();
...
int cg_aanext(const int arytemp) {
  int t = cgalloctemp();
...
void cg_stridxcheck(const int idxtemp, const int basetemp, const int funcname) {
  int t1;
  int zerotemp = cgalloctemp();
...
int cg_arrayiterator(const ASTnode * n, const Breaklabel *this) {
  ASTnode *assign;
  int Lfortop;
  int aryptr;
  int idx;
...
int cg_copystruct(const int srctemp, const int desttemp, const int size) {
  int t= cgalloctemp();
