...
void function_declaration(ASTnode * func, const int visibility);
ASTnode *function_prototype(const ASTnode * func);
ASTnode *typed_declaration_list(void);
void enum_declaration(void);
void struct_declaration(string name);
ASTnode *union_declaration(void);
int get_visibility(void);
void global_var_declaration(const ASTnode * decl, const int visibility);
ASTnode *decl_initialisation(void);
ASTnode *array_typed_declaration(void);
ASTnode *typed_declaration(void);
void funcptr_declaration(string typename);
int *array_size(inout int num);
Type *assoc_keytype(void);
Type *match_type(const bool checkonly);
ASTnode *statement_block(const Sym * func);
...
// Parse and return either an integer NUMLIT or an enum value
int64 integer_constant(void) {
  Sym *sym;
  int64 val;
  ...
  switch(Thistoken.token) {
    case T_NUMLIT:
      if (Thistoken.litval.numtype == NUM_FLT)
        fatal("Cannot use a float literal here\n");
      val= Thistoken.litval.intval;
    case T_IDENT:
      sym= find_symbol(Thistoken.tokstr);
      if (sym == NULL || sym.symtype != ST_ENUM)
        fatal("Unrecognised enum name %s\n", Thistoken.tokstr);
      val= sym.count;
    default:
      fatal("Need either an integer literal value or an enum name\n");
  }
...
void type_declaration(void) {
  ...
  if (Thistoken.token == T_ASSIGN) {
    ...
    if (Thistoken.token == T_FUNCPTR) {
      ...
      if (basetype == NULL)
	fatal("Unknown base type in type declaration for %s\n",
	      typename);
      ...
      if (Thistoken.token == T_RANGE) {
        scan(Thistoken);
        lower= integer_constant();
        match(T_ELLIPSIS, true);
        upper= integer_constant();
      ...
      ty= new_type(TY_USER, basetype.size,
...
void funcptr_declaration(string typename) {
  ...
  if (Thistoken.token == T_THROWS) {
    ...
    if ((excepttype.kind != TY_STRUCT) || (excepttype.ptr_depth != 1))
      fatal("Type %s not suitable to hold an exception\n",
            get_typename(excepttype));
    basetype= value_at(excepttype);
    if ((basetype.memb == NULL) || (basetype.memb.ty != ty_int32))
      fatal("Type %s not suitable to hold an exception\n",
            get_typename(excepttype));
  }
...
int add_memb_to_struct(Type * strtype, ASTnode * asthead,
  ...
  foreach astmemb (asthead, astmemb.mid) {
    ...
    foreach i (0 ... astmemb.dimensions - 1) {
...
void global_var_declaration(const ASTnode * decl, const int visibility) {
  ...
  if (Thistoken.token == T_ASSIGN) {
    ...
    init= check_bel(sym, init, 0, false, NOTEMP);
    if (init != NULL)
...
void function_declaration(ASTnode * func, const int visibility) {
  ...
  if (func.ty != ty_void && value_returned==false)
...
int *array_size(inout int num) {
  ...
  while (i < max) {
    ...
    // Get the dimension's size, skip the ']'
    list[i] = cast(count, int32);
...
ASTnode *short_assign_stmt(void) {
  ...
  if (v.op == A_DEREF && v.left.ty == ty_string)
...
int hididx= 0;
string new_idxvar(void) {
  string name= Malloc(20);
...
ASTnode *function_call(void) {
  ...
  if (sym == NULL ||
      (sym.symtype != ST_FUNCTION && sym.ty.kind != TY_FUNCPTR))
...
ASTnode *postfix_variable(ASTnode * n) {
  ...
  case T_IDENT:
    ...
    if (n.sym.is_inout) {
      ...
      if ((n.sym.ty.kind != TY_STRUCT) ||
          (n.sym.ty.ptr_depth != 1)) {
        n= mkastnode(A_DEREF, n, NULL, NULL);
        n.ty= value_at(n.left.ty);
        n.rvalue= true;
      }
    }
