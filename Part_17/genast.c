...
static void gen_checkparams(ASTnode * n);
...
void gen_local(ASTnode * n) {
  ...
  // A function pointer, set size to 8
  if (n->sym->symtype == ST_FUNCPOINTER)
    size = 8;
...
static int gen_funccall(ASTnode * n) {
  ...
  func = n->sym;
  if (func->symtype != ST_FUNCTION && func->symtype != ST_FUNCPOINTER)
...
// Given an ASSIGN node with a
// function pointer on the right,
// compare the left/right parameters
// to ensure both function prototypes
// match. Yes, I should refactor this
// with the code in funcs.c
static void gen_checkparams(ASTnode * n) {
  Sym *func= n->left->sym;
  Sym *funcptr= n->right->sym;
  Sym *lparam;
  Sym *rparam;
  int count=1;

  if (func == NULL || func->symtype != ST_FUNCTION)
    fatal("Can only assign a function's name to a function pointer\n");

  lparam= func->paramlist;
  rparam= funcptr->paramlist;

  while (lparam != NULL || rparam != NULL) {
    if (lparam == NULL || rparam == NULL)
      lfatal(n->line, "Inconsistent parameter count: %s vs. %s()\n",
		funcptr->name, func->name);

    if (rparam->type != lparam->type)
      lfatal(n->line, "Parameter %d in %s not consistent with %s(): %s vs %s\n",
	count, funcptr->name, func->name,
	get_typename(rparam->type), get_typename(lparam->type));
