...
bool add_function(ASTnode * func, ASTnode * paramlist, int visibility) {
  ...
  if (func->is_funcpointer)
    funcptr = add_symbol(func->strlit, ST_FUNCPOINTER, func->type, visibility);
  else
  ...
  // The function is a new one.
  // Walk the parmlist adding each name
  // and type to the function's member list
