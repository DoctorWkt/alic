...
// was escaped
static int scanch(bool *was_escaped) {
  int i, c, c2;
...
static void scan_numlit(Token * t, int c, bool is_negative) {
  ...
  // Do the conversion
  if (isfloat)
    t->litval.dblval = atof(Text);
  else
    t->litval.uintval = atol(Text);
}
...
void dumptokens(void) {
  ...
  while (1) {
	...
	fprintf(Debugfh, " '%c'", (int) t.litval.intval);
	break;
      }
      // fallthrough
    case T_IDENT:
