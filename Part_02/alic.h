...
#ifdef CPU_pdp11
#define PTR_SIZE 2		// Pointer size in bytes
#define PTR_TYPE ty_uint16	// Equivalent int type for pointers
#else
#define PTR_SIZE 8		// Pointer size in bytes
#define PTR_TYPE ty_uint64	// Equivalent int type for pointers
#endif

#ifdef CPU_pdp11
#define Intsize int32_t
#define UIntsize uint32_t
#else
#define Intsize int64_t
#define UIntsize uint64_t
#endif
...
struct Type {
  ...
  Intsize lower;		// For user-defined integer types, the range of
  Intsize upper;		// the type. If lower==upper==0, no range
...
struct Litval {
  union {
    Intsize i;			// Signed integer
    UIntsize u;			// Unsigned integer
...
enum {
  ...
  // These used only in ILnodes
  A_FUNCPREAMBLE, A_FUNCPOSTAMBLE,				// 63
  A_LOADLIT, A_LOADBOOL, A_LOADVAR,				// 65
  A_STORVAR, A_STORDEREF, A_STRLITVAL,				// 68
  A_GENLABEL, A_JUMP, A_JUMPIFFALSE,				// 71
  A_GLOBSYM, A_GLOBSYMVAL, A_GLOBSYMEND,			// 74
  A_STORELEM, A_MOVE, A_RANGE, A_COPYSTRUCT,			// 77
  A_STRHASH, A_INITTEMP, A_INCTEMP,				// 81
  A_AAGETVAL, A_AASETVAL, A_AAEXISTS, A_AADELVAL,		// 84
  A_MISC, A_FITERNEXT, A_STRINDEX, A_SITERNEXT			// 88
};
...
extern FILE *Interfh;		// The IL file handle
...
extern Intsize typemin[8];	// Minimum values per type
extern Intsize typemax[8];	// Maximum values per type
