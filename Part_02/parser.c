...
static Intsize integer_constant(void) {
  Sym *sym;
  Intsize val;
...
static void funcptr_declaration(char *typename) {
  ...
  if (Thistoken.token == T_THROWS) {
    ...
    if ((basetype->memb == NULL) || (basetype->memb->type != ty_int32))
      fatal("Type %s not suitable to hold an exception\n",
            get_typename(excepttype));
  }
...
static int *array_size(int *num) {
  ...
  while (i < max) {
    ...
    // We need an count value which isn't negative
    if (count < 0)
...
static ASTnode *additive_expression(void) {
  ...
  while (loop) {
	...
	right= mkastleaf(A_NUMLIT, PTR_TYPE, true, NULL, typesize);
...
static ASTnode *postfix_variable(ASTnode * n) {
  ...
  case T_LBRACKET:
    ...
    if (is_pointer(n->type)) {
      ...
      off = mkastleaf(A_NUMLIT, PTR_TYPE, true, NULL, ty->size);
  ...
  case T_DOT:
    ...
    off = mkastleaf(A_NUMLIT, PTR_TYPE, true, NULL, memb->offset);
