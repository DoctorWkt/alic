...
bool O_keepasm = false;		// Keep the intermediate IL & asm code
...
static char *do_compile(char *filename) {
  ...
  sprintf(cmd, "%s %s %s", CPPCMD, INCDIR, filename);
  ...
  if ((Outfh = fopen("/dev/null", "w")) == NULL) {
    fprintf(stderr, "Unable to write to /dev/null: %s\n", strerror(errno));
    exit(1);
  }

  // XXX Temporary: create an intermediate file
  Interfilename = alter_suffix(filename, 'i');
  if ((Interfh = fopen(Interfilename, "w")) == NULL) {
  ...
  fclose(Interfh);		// Close the IL file
...
// Given an input filename, run il2asm on the file and
// produce an assembly file. Return the assembly filename
char *do_il2asm(char *filename) {
  char cmd[TEXTLEN];
  int err;
  ...
  // Build the il2asm command and run it
  sprintf(cmd, "%s %s", IL2ASM, filename);
  if (O_verbose)
    fprintf(stderr, "%s\n", cmd);
  err = system(cmd);

  if (err != 0) {
    fprintf(stderr, "il2asm translation of %s failed\n", filename);
    exit(1);
  }
...
void do_link(char *outfilename, char **objlist) {
  ...
  cptr = cmd;
  cnt = sprintf(cptr, "%s %s ", LDCMD, outfilename);
  cptr += cnt;
  size -= cnt;

  // Now append each object file
  while (*objlist != NULL) {
    cnt = sprintf(cptr, "%s ", *objlist);
    cptr += cnt;
    size -= cnt;
    objlist++;
  }

  // Append the libalic.a location
  sprintf(cptr, "%s", LIBALIC);
...
int main(int argc, char *argv[]) {
  ...
  while (optind < argc) {
    ilfile = do_compile(argv[optind]);	// Compile the source file
    asmfile = do_il2asm(ilfile);	// Create the assembly file
    ...
    if (!O_keepasm) {		// Remove the IL and assembly files
      unlink(ilfile);		// if we don't need to keep them
      unlink(asmfile);
    }
    optind++;
  }
