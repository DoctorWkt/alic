...
static Type* type(void) {
  ...
  while (Thistoken.token== T_STAR) {
    scan(&Thistoken); depth++;
  }

  // No pointer depth
  if (depth==0) return(t);

  // Find a matching pointer to the type in the list.
  // Add it if it doesn't exist
  newt= find_type(NULL, t->kind, depth);
  if (newt==NULL)
    // XXX We shouldn't use 8. Instead, we should have
    // a function that tells us the size of pointers
    // for the machine we are compiling down to
    newt= new_type(t->kind, 8, depth);
  return(newt);
}
